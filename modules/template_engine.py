...
class AFTemplateEngine:
    ...
    def _enable_field_update_on_open(self):
        """
        Insert <w:updateFields w:val="true"/> into document settings so that
        the Table of Contents and all field codes auto-refresh on open.

        Uses "true" (not "1") for broadest Word/LibreOffice compatibility.
        The ZIP post-processor also patches settings.xml directly as a
        belt-and-suspenders guarantee (python-docx settings API can be fragile).
        """
        try:
            settings_el = self.doc.settings.element
            for old in settings_el.findall(qn('w:updateFields')):
                settings_el.remove(old)
            uf = parse_xml('<w:updateFields {} w:val="true"/>'.format(nsdecls("w")))
            settings_el.insert(0, uf)
        except Exception:
            pass  # non-fatal; ZIP post-processor handles it too
    ...
    def _postprocess_zip(self, docx_bytes: bytes) -> bytes:
        ...
        with zipfile.ZipFile(in_buf, 'r') as zin,              zipfile.ZipFile(out_buf, 'w', zipfile.ZIP_DEFLATED) as zout:
            for item in zin.infolist():
                ...
                # ── Belt-and-suspenders: ensure updateFields in settings.xml ─
                elif fname == 'settings.xml' and item.filename.startswith('word/'):
                    try:
                        s = data.decode('utf-8')
                        # Remove any existing (possibly wrong) updateFields
                        s = re.sub(r'<w:updateFields[^/]*/>', '', s)
                        # Insert fresh one before closing tag
                        s = s.replace(
                            '</w:settings>',
    ...
    def _fix_hf_xml_part(xml_bytes: bytes, ctx: dict, fname: str) -> bytes:
        ...
        1. Resolve {{ }} fields at PARAGRAPH level (not per-<w:t>).
           Word's spell-checker fragments "{{ document_no }}" across many runs:
             "{{" | " " | "document" | "_" | "no" | " }}" (separate <w:r>/<w:t>).
           Individual <w:t> scanning misses these patterns completely.
           We collect all text within each <w:p>, check for {{ }}, resolve the
           combined string, then replace the paragraph's run content with one
           clean run — preserving run-properties from the first original run
           and keeping any fldChar runs intact (PAGE / NUMPAGES fields).

        2. Footer: convert fixed-DXA table widths to percentage (auto-fit to
           any page width — A4, A3 portrait, A3 landscape).

        3. Header: bump right-tab stop to A3-landscape content width so the
           client name aligns to the right margin on ALL page sizes.
        """
        root = _etree.fromstring(xml_bytes)

        # ── 1. Paragraph-level field resolution ─────────────────────────────
        for para in root.iter(f'{{{_W_NS}}}p'):
            # Collect full text from ALL <w:t> descendants in this paragraph
            full_text = ''.join(
                (t.text or '') for t in para.iter(f'{{{_W_NS}}}t')
            )
            if '{{' not in full_text:
                continue
            resolved = AFTemplateEngine._resolve_jinja2(full_text, ctx)
            if resolved == full_text:
                continue  # no change needed

            # Preserve: pPr (paragraph properties) and fldChar runs (PAGE/NUMPAGES)
            pPr      = para.find(f'{{{_W_NS}}}pPr')
            first_r  = para.find(f'{{{_W_NS}}}r')
            first_rPr = (copy.deepcopy(first_r.find(f'{{{_W_NS}}}rPr'))
                         if first_r is not None else None)
            fld_runs = [
                c for c in list(para)
                if c.tag == f'{{{_W_NS}}}r' and (
                    c.find(f'{{{_W_NS}}}fldChar') is not None or
                    c.find(f'{{{_W_NS}}}instrText') is not None
                )
            ]
            ...
            # Restore fldChar runs (page number fields, etc.)
            for fr in fld_runs:
                para.append(fr)

            # Add a single run with the resolved text (skip if only fields remain)
            if not fld_runs:
                r_elem = _etree.SubElement(para, f'{{{_W_NS}}}r')
                if first_rPr is not None:
                    r_elem.append(first_rPr)
                t_elem = _etree.SubElement(r_elem, f'{{{_W_NS}}}t')
        ...
        if 'footer' in fname:
            ...
            for idx2, tcW in enumerate(tc_widths):
                w = per_col_pct + (remainder if idx2 == len(tc_widths) - 1 else 0)
    ...
    def _insert_logos(self):
        """
        Insert logos into {{consultant_logo}} / {{client_logo}} placeholders.

        Problem: Word's spell-checker splits {{field}} placeholders with
        <w:proofErr> elements, turning {{consultant_logo}} into three separate
        runs: "{{" | (proofErr) | "consultant_logo" | (proofErr) | "}}".
        Checking run.text never finds the full placeholder.

        Fix: check para.text (which joins all run texts), then clear the entire
        paragraph (removing split runs + proofErr fragments) and insert a fresh
        run containing the picture. Para formatting (pPr) is preserved.
        """
        def _clear_para_keep_ppr(para):
            """Remove every child of <w:p> except <w:pPr>."""
            p = para._p
            for child in list(p):
                tag = child.tag.split('}')[-1] if '}' in child.tag else child.tag
                if tag != 'pPr':
            ...
            for row in table.rows:
                for cell in row.cells:
                    for para in cell.paragraphs:
                        pt = para.text  # joins all runs, ignores proofErr
                        if "{{consultant_logo}}" in pt:
                            _clear_para_keep_ppr(para)
                            if self.consultant_logo and os.path.exists(self.consultant_logo):
                                para.add_run().add_picture(
                                    self.consultant_logo, width=Inches(2.0))
                            # else: fallback text injected via self.fields below
                        elif "{{client_logo}}" in pt:
                            _clear_para_keep_ppr(para)
                            if self.client_logo and os.path.exists(self.client_logo):
                                para.add_run().add_picture(
                                    self.client_logo, width=Inches(2.0))
